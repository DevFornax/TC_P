...
const SwitchSldPrint = forwardRef(({ thermalInspection = {} }, ref) => {
  const scale = SwitchScale;
  ...
  return (
    ...
    >
        ...
        {Object.entries(thermalInspection).length === 0 ? (
          ...
          Object.entries(thermalInspection).map(([id, status], index) => (
            <text
              ...
              y={30 + index * 20}
              fontSize="14"
              fill={status === "H" ? "red" : status === "M" ? "green" : "black"}
            >
              {id}:{" "}
              {status === "H" ? "High" : status === "M" ? "Medium" : "No Data"}
            </text>
          ))
        )}
      </g>
      ...
      <g transform={`translate(${dragOffset.x}, ${dragOffset.y})`}>
        ...
        {switchWaypoints.map((point) => {
          if (!switchVisiblePointIds.includes(point.id)) return null;

          const [x, y] = getScaled(point.coordinates);
          const Icon = switchCustomIcons[point.id];
